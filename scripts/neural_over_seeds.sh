...
do
    ...
    bash scripts/mumford_eval.sh learning/eval_route_generator.py \
        ...
        experiment/cost_function=mine model=bestsofar_feb2023 \
        +model.weights=output/inductive_seed_${seed}.pt \
        experiment.seed=$seed +run_name=balanced_seed_$seed \
        >> neural_seeds_balanced.csv
    bash scripts/mumford_eval.sh learning/eval_route_generator.py \
        eval_model_mumford hydra/job_logging=disabled \
        experiment/cost_function=mine_pp model=bestsofar_feb2023 \
        +model.weights=output/inductive_seed_${seed}.pt \
        experiment.seed=$seed +run_name=pp_seed_$seed \
        >> neural_seeds_pp.csv
    bash scripts/mumford_eval.sh learning/eval_route_generator.py \
        eval_model_mumford hydra/job_logging=disabled \
        experiment/cost_function=mine_op model=bestsofar_feb2023 \
        +model.weights=output/inductive_seed_${seed}.pt \
