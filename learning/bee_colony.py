...
from torch_utils import reconstruct_all_paths, aggr_edges_over_sequences, \
    get_update_at_mask, get_batch_tensor_from_routes, get_unselected_routes
...
def bee_colony(state, cost_obj, n_bees=10, passes_per_it=5, 
               mod_steps_per_pass=2, shorten_prob=0.2, n_iterations=400, 
               n_type1_bees=None, n_type2_bees=None, silent=False, 
               init_scenario=None, force_linking_unlinked=False, bee_model=None, 
               sum_writer=None):
    ...
    def batched_cost_fn(bee_scenarios):
        bee_states.replace_routes(bee_scenarios.flatten(0,1))
    ...
    bee_costs, bee_metrics = batched_cost_fn(bee_scenarios)
    ...
    for iteration in tqdm(range(n_iterations), disable=silent):
        for pi in range(passes_per_it):
            for mi in range(mod_steps_per_pass):
                ...
                rands = torch.rand(size=(batch_size, n_bees), device=dev)
                chosen_route_idxs = (rands * n_routes[:, None]).long()
                ...
                new_bee_scenarios = \
                    get_mutants(bee_scenarios, chosen_route_idxs, n_type1_bees, 
                ...
                new_bee_costs, new_bee_metrics = \
                    batched_cost_fn(new_bee_scenarios)
            ...
            best_scenarios[is_improvement] = \
                bee_scenarios[is_improvement, improvement_idx]
            ...
            # update the scenarios and costs of the followers
    ...
    state.replace_routes(best_scenarios)
...
def get_direct_sat_dmd(stop_dmd, shortest_paths, symmetric):
    ...
    summed_demands = aggr_edges_over_sequences(shortest_paths, stop_dmd[..., None])
    if symmetric:
        summed_demands += \
...
def get_mutants(bee_scenarios, chosen_route_idxs, n_type1, n_type2,
    ...
    scen_idxs = torch.randperm(n_bees, device=bee_scenarios.device)
    ...
    if bee_model is not None:
        # run it on all bees...
        new_type1_scenarios = get_neural_variants(bee_model, env_state, 
                                                  bee_scenarios,
                                                  chosen_route_idxs)
        # ...and keep only the type 1 bee routes
        new_type1_routes = new_type1_scenarios[:, type1_idxs, -1]
    else:
        unsel_routes = get_unselected_routes(bee_scenarios, chosen_route_idxs)
    ...
    if rpc_model is not None:
        ...
        new_type3_scenarios = get_neural_variants(rpc_model, env_state,
                                                  bee_scenarios,
                                                  chosen_route_idxs)
        new_type3_routes = new_type3_scenarios[:, type3_idxs, -1]
...
def get_bee_1_variants(remaining_state, batch_bee_routes, direct_sat_dmd_mat,
    ...
    if force_linking_unlinked:
        # if there are any unlinked node pairs, consider only new routes that link
...
def main(cfg: DictConfig):
    ...
    init_soln_file = cfg.get('init_solution_file', None)
    fixed_routes_file = cfg.get('fixed_routes_file', None)
    draw = cfg.eval.get('draw', False)

    nt1b = cfg.get('n_type1_bees', None)
    nt2b = cfg.get('n_type2_bees', None)
    routes = \
        lrnu.test_method(bee_colony, test_dl, cfg.eval, cost_fn, 
            sum_writer=sum_writer, silent=False,
            n_bees=cfg.n_bees, n_iterations=cfg.n_iterations,
            n_type1_bees=nt1b, n_type2_bees=nt2b, 
            init_solution_file=init_soln_file, device=DEVICE, 
            bee_model=bee_model, return_routes=True,
