...
def train_ppo(model, min_n_routes, max_n_routes, cfg, optimizer, 
    ...
    route_by_route = cfg.model.route_generator.kwargs.get('route_by_route', 
                                                          False)
    ...
    for iteration in range(cfg.ppo.n_iterations):
        ...
        if route_by_route:
            buf_actions = torch.full((horizon, state.batch_size, 
                                      state.max_n_nodes, 2), -1,
                                     device=device, dtype=torch.long)
        else: 
        ...
        with torch.no_grad():    
            for tt in range(horizon):
                ...
                if route_by_route:
                    ...
                    actions, logits, _ = model.plan_new_route(state)
                    buf_actions[tt, :, :actions.shape[1]] = actions
                else:
        ...
        for idxs in torch.split(train_order, n_states_per_minibatch):
            ...
            # step model forward
            if route_by_route:
                _, logits, entropy = model.plan_new_route(mb_states.clone(),
                                                          actions=mb_acts)
            else:
...
def train(model, min_n_routes, max_n_routes, cfg, optimizer, train_dataloader, 
    ...
    pbar = tqdm(total=graphs_per_epoch * cfg.n_epochs)
    ...
    for epoch in range(cfg.n_epochs):
        ...
        for _ in range(batches_per_epoch):
            ...
            while not state.is_done().all():
                n_routes_so_far.append(state.n_routes_so_far)
                state_vecs.append(state.get_global_state_features())
                if route_by_route:
                    # in this mode, the state gets updated internally
                    actions, logits, entropy = model.plan_new_route(state)
                else:
            ...
            if bl_module is not None:
                # if type(bl_module) is NNBaseline:
                #     baseline = bl_module.from_state(state)
                # else:
