...
class RouteGeneratorBase(nn.Module):
    ...
    def forward(self, state, greedy=False):
        ...
        routes_tensor = get_batch_tensor_from_routes(state.routes, 
                                                     state.device)
...
class PathCombiningRouteGenerator(RouteGeneratorBase):
    def __init__(self, *args, n_pathscorer_layers=3, pathscorer_hidden_dim=16,
                 ...
                 serial_halting=True, route_by_route=False, max_act_len=None,
                 **kwargs):
        ...
        serial_halting -- if True, the halt score is used to make a binary
            ...
            extensions.
        route_by_route -- if True, model.forward() recomputes the nodepair 
            scores only when starting a new route, reusing them for each action 
            while planning that route, and also updates the state internally.  
            If False, it recomputes nodepair scores before every action, and 
            does not update the state internally.
        """
        super().__init__(*args, **kwargs)
        self.logit_clip = logit_clip
        self.serial_halting = serial_halting
        self.route_by_route = route_by_route
    ...
    def forward(self, state, greedy=False):
        ...
        while not state.is_done().all():
            if self.route_by_route:
                # in this mode, the state gets updated internally
                _, logits, entropy = self.plan_new_route(state)
    ...
    def plan_new_route(self, state, greedy=False, actions=None):
        # generate the input batch
        encoding = self._encode_graph(state)
        init_n_routes = state.n_routes_so_far
    ...
    def step(self, state, greedy=False, actions=None,
             precalc_data=None):
        ...
        paths_are_invalid = paths_are_invalid
        ...
        if self.force_linking_unlinked:
            ...
            # only apply this limitation if there are are some options that
             # can extend coverage
            can_extend_coverage = \
                (extends_coverage_if_needed & ~paths_are_invalid).\
                any(-1).any(-1)
            fails_to_extifneeded = ~extends_coverage_if_needed
            update = fails_to_extifneeded & can_extend_coverage[:, None, None]
            
            paths_are_invalid |= update
        ...
        route_is_done = state.is_done() | no_valid_options
        ...
        halt_scores[route_is_done] = TORCH_FMAX
        ...
        from_idxs = flat_idxs // state.n_nodes
    ...
    def _encode_graph(self, state):
        ...
        path_scores = aggr_edges_over_sequences(path_seqs, 
                                                np_scores[..., None], 'sum')
        path_scores.squeeze_(-1)

        return node_descs, node_pad_mask, np_embeds, path_scores

    def _get_extension_scores(self, state, route_lens, are_on_routes, 
                              all_paths, nodepair_embeds, drive_times, 
                              path_scores, path_lens, invalid_terms,
                              before_or_after='after'):
        ...
        out_scores = get_update_at_mask(out_scores, ~out_valids, TORCH_FMIN)
